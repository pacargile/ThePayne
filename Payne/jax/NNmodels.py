...
class SMLP(nn.Module):
    def __init__(self, D_in, H1, H2, H3, D_out, xmin, xmax):
        ...
        self.w1 = self.features[0].weight.detach().numpy()
        self.w2 = self.features[2].weight.detach().numpy()
        self.w3 = self.features[4].weight.detach().numpy()

        self.b1 = self.features[0].bias.detach().numpy()
        self.b2 = self.features[2].bias.detach().numpy()
        self.b3 = self.features[4].bias.detach().numpy()
    ...
    def npeval(self,x):
        x_i = self.npencode(x)
        L1      = np.einsum('ij,j->i', self.w1, x_i)                 + self.b1
        L2      = np.einsum('ij,j->i', self.w2, self.leaky_relu(L1)) + self.b2
        modspec = np.einsum('ij,j->i', self.w3, self.leaky_relu(L2)) + self.b3
        return modspec
...
class LinNet(nn.Module):  
    def __init__(self, D_in, H1, H2, H3, D_out, xmin, xmax):
        ...
        self.w1 = self.lin1.weight.detach().numpy()
        self.w2 = self.lin2.weight.detach().numpy()
        self.w3 = self.lin3.weight.detach().numpy()
        self.w4 = self.lin4.weight.detach().numpy()
        self.w5 = self.lin5.weight.detach().numpy()
        self.w6 = self.lin6.weight.detach().numpy()

        self.b1 = self.lin1.bias.detach().numpy()
        self.b2 = self.lin2.bias.detach().numpy()
        self.b3 = self.lin3.bias.detach().numpy()
        self.b4 = self.lin4.bias.detach().numpy()
        self.b5 = self.lin5.bias.detach().numpy()
        self.b6 = self.lin6.bias.detach().numpy()
    ...
    def npencode(self,x):
        # convert x into numpy to do math
        x_np = np.array(x)
        xout = (x-self.xmin)/(self.xmax-self.xmin) - 0.5
        return xout

    def npeval(self,x):
        x_i = self.npencode(x)
        L1      = np.einsum('ij,j->i', self.w1, x_i)              + self.b1
        L2      = np.einsum('ij,j->i', self.w2, self.sigmoid(L1)) + self.b2
        L3      = np.einsum('ij,j->i', self.w3, self.sigmoid(L2)) + self.b3
        L4      = np.einsum('ij,j->i', self.w4, self.sigmoid(L3)) + self.b4
        L5      = np.einsum('ij,j->i', self.w5, self.sigmoid(L4)) + self.b5
        modspec = np.einsum('ij,j->i', self.w6, self.sigmoid(L5)) + self.b6
        return modspec
