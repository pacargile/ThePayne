...
class TrainMod(object):
     ...
     def __init__(self, *arg, **kwargs):
          ...
          spectra_test_arr = []
          for spectra_test_i,continuua_test_i in zip(spectra_test,continuua_test):
               # multiply spectrum by continuum
               spectra_test_i = spectra_test_i * continuua_test_i
               # divide fluxed spectrum by median
               spectra_test_i = spectra_test_i / np.nanmedian(spectra_test_i)
               spectra_test_arr.append(spectra_test_i)
          spectra_test = np.array(spectra_test_arr)
     ...
     def train_mod(self):
          ...
          for epoch_i in range(int(self.numepochs)):
               ...
               spectra_train_arr = []
               for spectra_train_i,continuua_train_i in zip(spectra_train,continuua_train):
                    # multiply spectrum by continuum
                    spectra_train_i = spectra_train_i * continuua_train_i
                    # divide fluxed spectrum by median
                    spectra_train_i = spectra_train_i / np.nanmedian(spectra_train_i)
                    spectra_train_arr.append(spectra_train_i)
               spectra_train = np.array(spectra_train_arr)
               ...
               for spectra_valid_i,continuua_valid_i in zip(spectra_valid,continuua_valid):
                    # multiply spectrum by continuum
                    spectra_valid_i = spectra_valid_i * continuua_valid_i
                    # divide fluxed spectrum by median
                    spectra_valid_i = spectra_valid_i / np.nanmedian(spectra_valid_i)
                    spectra_valid_arr.append(spectra_valid_i)
               spectra_valid = np.array(spectra_valid_arr)
               ...
               for iter_i in range(int(self.numsteps)):
                    ...
                    if iter_i % 100 == 0:
                         ...
                         if self.logplot:
                              ...
                              fig.savefig('flux_loss_epoch{0}.png'.format(epoch_i+1),dpi=150)
                              plt.close(fig)
